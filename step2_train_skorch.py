...
train_inp, dev_inp = data_input[train_idx,:], data_input[dev_idx,:]
train_oup, dev_oup = data_output[train_idx,:], data_output[dev_idx,:]
...
# read data
x_train = train_inp.astype(np.float32)
y_train = train_oup.astype(np.float32)
x_dev = dev_inp.astype(np.float32)
y_dev = dev_oup.astype(np.float32)
x_test = test_inp.astype(np.float32)
y_test = test_oup.astype(np.float32)
...
net_regr = NeuralNetRegressor(
    Net(hidden_size=500,input_size = x_train.shape[1],output_size = y_train.shape[1]),
...
#if loadtrainedmodel:
#    model.load_state_dict(torch.load('step2_model_trained.pkl'))
#
## Loss and Optimizer
...
#    plt.title(titles[i])
#lgnd = plt.legend(loc='center left', bbox_to_anchor=(1, 1.15))
#for handle in lgnd.legendHandles:
#    handle.set_sizes([30.0])
#lossmessgae = "MSEs are:\n"+"Training set: "+str(train_error)+"\nDevelop set: "+str(dev_error)+"\nTest set      : "+str(test_error)
#plt.annotate(lossmessgae, xy=(1.05, 0.6), xycoords='axes fraction')
